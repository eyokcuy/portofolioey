<section id="feedback" class="py-24 px-6 md:px-32 relative overflow-hidden">
    <!-- Header -->
    ...
    <div id="feedback-list-container" class="relative w-full overflow-hidden py-10">
        @fragment('feedback-list')
            @if($feedbacks->count() > 0)
                <div class="flex gap-8 animate-scroll-horizontal hover:pause-animation">
                    @foreach($feedbacks->merge($feedbacks) as $feedback) {{-- Duplicate for infinite effect --}}
                        <div class="flex-shrink-0 w-[300px] md:w-[400px] skill-card p-8 rounded-3xl relative">
    ...
    <script>
        window.refreshFeedbackList = async function() {
            try {
                const response = await fetch("{{ route('feedback.partial') }}");
                if (response.ok) {
                    const html = await response.text();
                    document.getElementById('feedback-list-container').innerHTML = html;
                }
            } catch (error) {
                console.error('Failed to refresh feedback list:', error);
            }
        };
    </script>
